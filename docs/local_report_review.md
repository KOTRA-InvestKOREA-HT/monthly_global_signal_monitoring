...
3. Investment `indicator_supported`: Each candidate's `id` selects its indicator from the list below; the candidate itself carries no indicator name or description. Is there a concrete event matching the definition the candidate's `id` selects? Generic financial figures, risk or forward-looking boilerplate and keywords alone are insufficient. A trailing "etc." in a definition below means events of the same kind, not an expanded scope. Apply these inclusions and exclusions:
