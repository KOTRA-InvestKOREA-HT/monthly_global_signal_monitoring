...
- 기업 귀속과 지표 사건이 확인됐지만 타겟 기술 연결·전조·투자 단계·근거 충분성 중 하나가 모자라 탈락한 투자 후보는 `ai_review_tier=human_review`와 `ai_review_gaps`를 달아 투자 시그널 파일과 PDF에 함께 싣는다. 사람이 대시보드에서 해당 없는 항목을 무시로 거른다. 이런 후보에도 승인 후보와 같은 형식의 `summary_ko`, `summary_en`을 작성한다(승인을 뜻하지 않는다). 이 구간은 판정 계약 밖이라 모델에 보내지 않는다.
