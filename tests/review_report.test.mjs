...
test('requires explicit free-tier confirmation and bounds API requests', () => {
  ...
  assert.deepEqual(configuration({ OPENAI_API_KEY: 'key' }), { apiKey: 'key', maxRequests: 400, delayMs: 1600, concurrency: 12 });
  ...
  assert.deepEqual(configuration({ OPENAI_API_KEY: 'key', NVIDIA_MAX_REQUESTS: '400', NVIDIA_DELAY_MS: '1600' }), { apiKey: 'key', maxRequests: 400, delayMs: 1600, concurrency: 12 });
});
